...
from models.queries import (
    ...
    create_evaluation, upsert_note, get_all_teachers, create_student_admin, 
    update_student_admin, delete_student_admin, create_teacher_admin, 
    update_teacher_admin, delete_teacher_admin
)
...
def student_notes():
    ...
    return render_template('dashboard_student.html', student=student_info, notes=notes, stats=stats, ranking=ranking)
...
def add_student():
    ...
    if create_student_admin(matricule, nom, prenom, sexe, date_naissance, telephone, nationalite, boursier, email, password):
        flash("Étudiant créé avec succès !", "success")
    else:
        flash("Erreur lors de la création de l'étudiant.", "danger")
    return redirect(url_for('dashboards.admin_students'))
...
def edit_student(etudiant_id):
    ...
    if update_student_admin(etudiant_id, matricule, nom, prenom, sexe, date_naissance, telephone, nationalite, boursier, email):
        ...
        flash("Erreur lors de la mise à jour de l'étudiant.", "danger")
    return redirect(url_for('dashboards.admin_students'))
...
def add_teacher():
    ...
    if create_teacher_admin(nom, prenom, grade, telephone, annee_embauche, statut, email, password):
        ...
        flash("Erreur lors de la création du professeur.", "danger")
    return redirect(url_for('dashboards.admin_students'))
...
def edit_teacher(prof_id):
    ...
    if update_teacher_admin(prof_id, nom, prenom, grade, telephone, annee_embauche, statut, email):
        flash("Professeur mis à jour avec succès !", "success")
    else:
        flash("Erreur lors de la mise à jour du professeur.", "danger")
    return redirect(url_for('dashboards.admin_students'))
...
def add_evaluation(ens_id):
    ...
    if not type_eval or not date_eval:
        ...
        flash("Le coefficient doit être strictement positif.", "danger")
    elif type_eval not in ('Examen', 'Expose', 'Projet', 'Rattrapage'):
        flash("Type d'évaluation non valide selon la base de données.", "danger")
    else:
