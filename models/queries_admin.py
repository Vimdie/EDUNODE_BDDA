...
def get_all_students():
    """Récupère tous les étudiants (pour l'admin) avec les infos Utilisateur, Classe et Filière."""
    conn = get_db_connection()
    if not conn: return []
    cursor = conn.cursor()
    query = """
        SELECT e.idEtudiant, e.Matricule, e.Nom, e.Prenom, e.Sexe, 
               e.DateNaissance, e.Telephone, e.Nationalite, e.Boursier,
               u.Email, c.NomClasse, f.NomFiliere, u.idUtilisateur
        FROM Etudiant e
    ...
    conn.close()
    return [{
        "id": row[0], "matricule": row[1], "nom": row[2], "prenom": row[3], "sexe": row[4], 
        "date_naissance": row[5], "telephone": row[6], "nationalite": row[7], "boursier": row[8],
        "email": row[9], "classe": row[10], "filiere": row[11], "user_id": row[12]
    } for row in students]

def create_student_admin(matricule, nom, prenom, sexe, date_naissance, telephone, nationalite, boursier, email, password):
    """Crée un utilisateur (rôle etudiant) puis l'étudiant associé."""
    conn = get_db_connection()
...
def update_student_admin(etudiant_id, matricule, nom, prenom, sexe, date_naissance, telephone, nationalite, boursier, email):
    """Met à jour les informations d'un étudiant et son email utilisateur."""
    conn = get_db_connection()
    if not conn: return False
    try:
        cursor = conn.cursor()
...
def delete_student_admin(etudiant_id):
    ...
    try:
        ...
        # Effacer Inscriptions / Notes liées à l'étudiant
        ...
        # Effacer l'étudiant puis l'utilisateur
...
def get_all_teachers():
    """Récupère tous les professeurs (pour l'admin) avec l'email."""
    conn = get_db_connection()
    if not conn: return []
    cursor = conn.cursor()
    query = """
        SELECT p.idProfesseur, p.NomProfesseur, p.PrenomProfesseur, p.GradeSpecialite,
               p.Telephone, p.AnneeEmbauche, p.Statu, u.Email, u.idUtilisateur
        FROM Professeur p
        JOIN Utilisateur u ON p.Utilisateur_idUtilisateur = u.idUtilisateur
    """
    cursor.execute(query)
    ...
    conn.close()
    return [{
        "id": row[0], "nom": row[1], "prenom": row[2], "grade": row[3],
        "telephone": row[4], "annee_embauche": row[5], "statut": row[6],
        "email": row[7], "user_id": row[8]
    } for row in teachers]

def create_teacher_admin(nom, prenom, grade, telephone, annee_embauche, statut, email, password):
    """Crée un utilisateur (rôle professeur) puis le professeur associé."""
    conn = get_db_connection()
    if not conn: return False
    try:
        cursor = conn.cursor()
...
def update_teacher_admin(prof_id, nom, prenom, grade, telephone, annee_embauche, statut, email):
    """Met à jour les infos d'un professeur et son email."""
    conn = get_db_connection()
    if not conn: return False
    try:
        cursor = conn.cursor()
...
def delete_teacher_admin(prof_id):
    ...
    try:
        ...
        # Le professeur peut être lié à des Enseignements, il faut les nettoyer ou bloquer
