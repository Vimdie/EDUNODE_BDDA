...
def get_student_info(user_id):
    """Récupère les infos d'un étudiant par son ID utilisateur."""
    conn = get_db_connection()
    if not conn: return None
    cursor = conn.cursor()
    query = "SELECT idEtudiant, Matricule, Nom, Prenom, Sexe, DateNaissance FROM Etudiant WHERE Utilisateur_idUtilisateur = ?"
    cursor.execute(query, (user_id,))
    student = cursor.fetchone()
    conn.close()
    if student:
        return {"id": student[0], "matricule": student[1], "nom": student[2], "prenom": student[3], "sexe": student[4], "date_naissance": student[5]}
    return None
...
def get_teacher_info(user_id):
    """Récupère les infos d'un professeur."""
    conn = get_db_connection()
    if not conn: return None
    cursor = conn.cursor()
    query = "SELECT idProfesseur, NomProfesseur, PrenomProfesseur, GradeSpecialite FROM Professeur WHERE Utilisateur_idUtilisateur = ?"
    cursor.execute(query, (user_id,))
    prof = cursor.fetchone()
    conn.close()
    if prof:
        return {"id": prof[0], "nom": prof[1], "prenom": prof[2], "grade": prof[3]}
    return None

def get_teacher_courses(prof_id):
    """Récupère les cours qu'un professeur enseigne."""
    conn = get_db_connection()
    if not conn: return []
    cursor = conn.cursor()
    query = """
        SELECT m.NomMatiere, c.NomClasse, p.Semestre, m.Description, ens.idEnseignement, p.VolumeHoraire, p.Credit
    ...
    conn.close()
    return [{"matiere": row[0], "classe": row[1], "semestre": row[2], "description": row[3], "id": row[4], "heure": row[5], "credit": row[6]} for row in courses]
...
def get_all_students():
    ...
    cursor = conn.cursor()
    query = """
    SELECT e.idEtudiant, e.Matricule, e.Nom, e.Prenom, e.Sexe, e.DateNaissance, e.Telephone, e.Nationalite, e.Boursier,
           u.Email, c.NomClasse, f.NomFiliere
    FROM Etudiant e
    LEFT JOIN Utilisateur u ON e.Utilisateur_idUtilisateur = u.idUtilisateur
    ...
    return [{
        "id": row[0],
        "matricule": row[1] if row[1] else '',
        "nom": row[2] if row[2] else '',
        "prenom": row[3] if row[3] else '',
        "sexe": row[4] if row[4] else '',
        "date_naissance": str(row[5]) if row[5] else '',
        "telephone": row[6] if row[6] else '',
        "nationalite": row[7] if row[7] else '',
        "boursier": row[8] if row[8] else '',
        "email": row[9] if row[9] else '',
        "classe": row[10] if row[10] else '',
        "filiere": row[11] if row[11] else ''
    } for row in students]

def get_all_teachers():
    """Récupère tous les professeurs (pour l'admin)."""
    conn = get_db_connection()
    if not conn: return []
    cursor = conn.cursor()
    query = """
    SELECT p.idProfesseur, p.NomProfesseur, p.PrenomProfesseur, p.GradeSpecialite, p.Telephone, p.AnneeEmbauche, p.Statu, u.Email
    FROM Professeur p
    LEFT JOIN Utilisateur u ON p.Utilisateur_idUtilisateur = u.idUtilisateur
    """
    cursor.execute(query)
    ...
    return [{
        ...
        "annee_embauche": row[5] if row[5] else '',
        "statut": row[6] if row[6] else '',
...
def get_student_program(etudiant_id):
    """
    R├®cup├¿re le programme acad├®mique complet de l'├®tudiant 
    (mati├¿res, semestre, volume horaire, cr├®dits, professeur).
    """
    conn = get_db_connection()
    if not conn: return []
    cursor = conn.cursor()
    ...
    return [{
        ...
        "professeur": f"{row[5]} {row[4]}" if row[4] and row[5] else "Non assign├®"
    } for row in program]

def get_student_credits(etudiant_id):
    """
    Calcule les cr├®dits valid├®s (notes >= 10 et statut 'Oui') vs les cr├®dits totaux du programme.
    """
    conn = get_db_connection()
    if not conn: return {"valides": 0, "total": 0, "pourcentage": 0}
    cursor = conn.cursor()
    
    # R├®cup├®rer le total des cr├®dits du programme de l'├®tudiant
    total_query = """
        SELECT SUM(p.Credit)
        FROM Inscription i
        JOIN Programme p ON i.Classe_idClasse = p.Classe_idClasse 
...
def update_student_profile_data(etudiant_id, telephone, sexe, date_naissance, nationalite):
    """Met ├á jour les informations de base de l'├®tudiant."""
    conn = get_db_connection()
...
def get_course_detail(ens_id):
    """Retourne les d├®tails complets d'un Enseignement (cours)."""
    conn = get_db_connection()
    if not conn: return None
    cursor = conn.cursor()
    cursor.execute("""
...
def get_course_evaluations(ens_id):
    """Retourne toutes les ├®valuations pour un Enseignement donn├®."""
    conn = get_db_connection()
    if not conn: return []
    cursor = conn.cursor()
    cursor.execute("""
...
def get_course_students_with_notes(ens_id, eval_id=None):
    """
    Retourne la liste des ├®tudiants inscrits dans la classe du cours,
    avec leur note pour une ├®valuation donn├®e (ou None si pas encore saisie).
    """
    conn = get_db_connection()
    if not conn: return []
    cursor = conn.cursor()
    # Trouver la classe li├®e ├á cet enseignement
    cursor.execute("""
...
def get_class_ranking_by_course(ens_id):
    """
    Calcule le classement des ├®tudiants de la classe
    bas├® sur la moyenne de toutes leurs notes dans ce cours.
    """
    conn = get_db_connection()
    if not conn: return []
    cursor = conn.cursor()
...
def get_class_note_distribution(ens_id):
    """Retourne la r├®partition des notes par plage pour Chart.js."""
    conn = get_db_connection()
    if not conn: return {}
    cursor = conn.cursor()
    cursor.execute("""
...
def create_evaluation(ens_id, type_eval, coefficient, date_eval):
    """Ins├¿re une nouvelle ├®valuation dans la table Evaluation."""
    conn = get_db_connection()
    if not conn: return False
    try:
        cursor = conn.cursor()
...
def upsert_note(inscription_id, eval_id, note_val, valide='oui', session='Normale'):
    """
    Ins├¿re ou met ├á jour une note.
    Si elle existe (m├¬me inscription + evaluation), UPDATE.
    Sinon, INSERT.
    """
    conn = get_db_connection()
    if not conn: return False
...
def create_student_admin(matricule, nom, prenom, sexe, date_naissance, telephone, nationalite, boursier, email, password):
    """Cr├®e un utilisateur (r├┤le etudiant) puis l'├®tudiant associ├®."""
    conn = get_db_connection()
    if not conn: return False
    try:
        cursor = conn.cursor()
        # Cr├®ation de l'utilisateur
        cursor.execute("INSERT INTO Utilisateur (Email, MotDePasse, Role) OUTPUT INSERTED.idUtilisateur VALUES (?, ?, 'etudiant')", (email, password))
        user_id = cursor.fetchone()[0]
        
        # Cr├®ation de l'├®tudiant
        cursor.execute("""
            INSERT INTO Etudiant (Utilisateur_idUtilisateur, Matricule, Nom, Prenom, Sexe, DateNaissance, Telephone, Nationalite, Boursier)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (user_id, matricule, nom, prenom, sexe, date_naissance, telephone, nationalite, boursier))
        conn.commit()
        return True
    except Exception as e:
        print(f"Erreur create_student_admin: {e}")
        conn.rollback()
        return False
    finally:
        conn.close()

def update_student_admin(etudiant_id, matricule, nom, prenom, sexe, date_naissance, telephone, nationalite, boursier, email):
    """Met ├á jour les informations d'un ├®tudiant et son email utilisateur."""
    conn = get_db_connection()
    if not conn: return False
    try:
        cursor = conn.cursor()
        ...
        cursor.execute("""
            UPDATE Etudiant 
            SET Matricule = ?, Nom = ?, Prenom = ?, Sexe = ?, DateNaissance = ?, Telephone = ?, Nationalite = ?, Boursier = ?
            WHERE idEtudiant = ?
        """, (matricule, nom, prenom, sexe, date_naissance, telephone, nationalite, boursier, etudiant_id))
        
        # Trouver l'utilisateur et MAJ email
        cursor.execute("SELECT Utilisateur_idUtilisateur FROM Etudiant WHERE idEtudiant = ?", (etudiant_id,))
        user_id = cursor.fetchone()[0]
        cursor.execute("UPDATE Utilisateur SET Email = ? WHERE idUtilisateur = ?", (email, user_id))
        
        conn.commit()
        return True
    except Exception as e:
        print(f"Erreur update_student_admin: {e}")
        conn.rollback()
        return False
    finally:
        conn.close()

def delete_student_admin(etudiant_id):
    """Supprime un ├®tudiant et son compte utilisateur."""
    conn = get_db_connection()
    if not conn: return False
    try:
        cursor = conn.cursor()
        ...
        # Effacer Inscriptions / Notes li├®es ├á l'├®tudiant
        cursor.execute("SELECT idInscription FROM Inscription WHERE Etudiant_idEtudiant = ?", (etudiant_id,))
        inscriptions = cursor.fetchall()
        for insc in inscriptions:
            cursor.execute("DELETE FROM Note WHERE Inscription_idInscription = ?", (insc[0],))
        cursor.execute("DELETE FROM Inscription WHERE Etudiant_idEtudiant = ?", (etudiant_id,))
        
        # Effacer l'├®tudiant puis l'utilisateur
        cursor.execute("DELETE FROM Etudiant WHERE idEtudiant = ?", (etudiant_id,))
        cursor.execute("DELETE FROM Utilisateur WHERE idUtilisateur = ?", (user_id,))
        
        conn.commit()
        return True
    except Exception as e:
        print(f"Erreur delete_student_admin: {e}")
        conn.rollback()
        return False
    finally:
        conn.close()

def create_teacher_admin(nom, prenom, grade, telephone, annee_embauche, statut, email, password):
    """Cr├®e un utilisateur (r├┤le professeur) puis le professeur associ├®."""
    conn = get_db_connection()
    if not conn: return False
    try:
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Utilisateur (Email, MotDePasse, Role) OUTPUT INSERTED.idUtilisateur VALUES (?, ?, 'professeur')", (email, password))
        user_id = cursor.fetchone()[0]
        
        cursor.execute("""
            INSERT INTO Professeur (Utilisateur_idUtilisateur, NomProfesseur, PrenomProfesseur, GradeSpecialite, Telephone, AnneeEmbauche, Statu)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (user_id, nom, prenom, grade, telephone, annee_embauche, statut))
        conn.commit()
        return True
    except Exception as e:
        print(f"Erreur create_teacher_admin: {e}")
        conn.rollback()
        return False
    finally:
        conn.close()

def update_teacher_admin(prof_id, nom, prenom, grade, telephone, annee_embauche, statut, email):
    """Met ├á jour les infos d'un professeur et son email."""
    conn = get_db_connection()
    if not conn: return False
    try:
        cursor = conn.cursor()
        cursor.execute("""
            UPDATE Professeur 
            SET NomProfesseur = ?, PrenomProfesseur = ?, GradeSpecialite = ?, Telephone = ?, AnneeEmbauche = ?, Statu = ?
            WHERE idProfesseur = ?
        """, (nom, prenom, grade, telephone, annee_embauche, statut, prof_id))
        
        cursor.execute("SELECT Utilisateur_idUtilisateur FROM Professeur WHERE idProfesseur = ?", (prof_id,))
        user_id = cursor.fetchone()[0]
        cursor.execute("UPDATE Utilisateur SET Email = ? WHERE idUtilisateur = ?", (email, user_id))
        ...
        print(f"Erreur update_teacher_admin: {e}")
        conn.rollback()
        return False
    finally:
        conn.close()

def delete_teacher_admin(prof_id):
    ...
    try:
        ...
        # Le professeur peut ├¬tre li├® ├á des Enseignements, il faut les nettoyer ou bloquer
        cursor.execute("DELETE FROM Enseignement WHERE Professeur_idProfesseur = ?", (prof_id,))
        cursor.execute("DELETE FROM Professeur WHERE idProfesseur = ?", (prof_id,))
        cursor.execute("DELETE FROM Utilisateur WHERE idUtilisateur = ?", (user_id,))
        
        conn.commit()
        return True
    except Exception as e:
        print(f"Erreur delete_teacher_admin: {e}")
        conn.rollback()
        return False
    finally:
        conn.close()